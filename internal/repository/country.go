...
func (r *CountryRepository) GetCountriesByRegion(ctx context.Context, regions []string) ([]*entity.Country, error) {
	...
	for _, region := range regions {
		query := `SELECT name, alpha2, alpha3, region FROM countries WHERE region = $1 ORDER BY alpha2 ASC`
...
// func (r *PostgresRepository) ListCountries(regions []string) ([]model.Country, error) {
// 	ctx, cancel := context.WithTimeout(context.Background(), operationTimeout)
// 	defer cancel()
...
// func (r *PostgresRepository) GetCountryByAlpha2(alpha2 string) (model.Country, error) {
// 	ctx, cancel := context.WithTimeout(context.Background(), operationTimeout)
// 	defer cancel()
